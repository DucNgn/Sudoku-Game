...
def solve(board):
    ...
    (subMatrixDigit, RowDigit, ColDigit, solvable) = setUpBitwises(temp)
    if(not solvable):
        return (temp, solvable)

    initPos = (0, 0)
    if(backtracking(temp, initPos, subMatrixDigit, RowDigit, ColDigit) is True):
        print('Solved')
    else:
        print('Unsolvable') 
        solvable = False
    ...
    for y in range(9):
        for x in range(9):
            if(board[y][x] > 0):
                ...
                if(existed(digitValue, (x, y), subMatrixDigit, RowDigit, ColDigit)):
                    return (subMatrixDigit, RowDigit, ColDigit, False)

                subMatrixDigit[int(y / 3)][int(x / 3)] |= digitValue
                RowDigit[y] |= digitValue
                ColDigit[x] |= digitValue

    return (subMatrixDigit, RowDigit, ColDigit, True)
...
def isBoardValid(board):
    (subMatrixDigit, RowDigit, ColDigit, solvable) = setUpBitwises(board)
    return solvable
...
def main():
    ...
    (result, solvable) = solve(board)
    if(solvable):
        printBoard(result, 9, 9)
    else:
        print("The Sudoku board cannot be solved")
